...
use kide_core::{
    ArtifactBlobCache, BuildSystem, CANONICAL_SCHEMA_VERSION, Completeness, Freshness, IndexStore,
    Precision, Provenance, QueryPayload, QueryProblem, QueryResponse, QueryStatus, ResultMetadata,
    SymbolId, WorkerCapability, WorkerInstallation, WorkerLaunch, WorkerRegistry, WorkspacePath,
    collect_workspace_text, discover_workspace, document_from_bytes,
...
fn run() -> Result<QueryStatus> {
    ...
    match cli.command {
        ...
        Command::Status => status(&cli.workspace, human_output),
...
fn status(workspace: &Path, human_output: bool) -> Result<QueryStatus> {
